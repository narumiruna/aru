...
pub fn discover(project: &Path, manifest: &Manifest) -> Result<Vec<DiscoveredInstruction>> {
    let inventory = inventory(project, false)?;
...
pub fn discover_agents(project: &Path) -> Result<Vec<String>> {
    let inventory = inventory(project, true)?;
...
fn inventory(project: &Path, onboarding: bool) -> Result<BTreeMap<String, InventoryEntry>> {
    ...
    let walker = WalkDir::new(project)
        ...
        .filter_entry(|entry| {
            ...
            !excluded_directory(relative, onboarding)
        });
    for item in walker {
        let item =
            item.map_err(|error| AruError::msg(format!("instruction discovery failed: {error}")))?;
        if item.depth() == 0 {
            continue;
        }
