...
An established Unix fallback directory is preferred only when it is owned by the effective UID and is private or contains existing lock/recovery state; unrelated fallback entries do not override a usable account home. This fallback is selected before inspecting the unused home-based state path, so a later home symlink does not block access to its lock or recovery journal. The selected fallback's own ancestors must still be free of symlinks. Owned recovery state is retained across permission changes; mutations repair its directory permissions and dry runs reject unsafe permissions.
