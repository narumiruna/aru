...
When the source repository contains a valid `aru.lock`, automatic discovery ignores a locked skill only under the hidden projection directories corresponding to that skill's locked targets, such as `.agents/skills/` or `.pi/skills/`.
This prevents installed project copies and their nested contents from affecting discovery while preserving untracked skills and skills under unrelated target directories; use `--path` to select a projection directory explicitly.
