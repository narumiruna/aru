...
`version = 1`. Each `skill-package` locks a normalized source, original requirement descriptor, selected SemVer or revision label, full 40-hex commit, repository root name, and selected `{name,path,sha256}` entries. Each `mcp-server` locks exact normalized metadata and one concrete target per selected agent.
