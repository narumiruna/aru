...
Otherwise, discovery traverses only the fixed roots implied by configured `files` selectors; a selector beginning with a wildcard necessarily starts at the project root.
Discovery always excludes `.git/**` and `.aru/**`; onboarding also skips common build/vendor roots and writes the exact files it found.
